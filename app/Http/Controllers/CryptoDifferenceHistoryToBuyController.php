...
class CryptoDifferenceHistoryToBuyController extends Controller
{
    /**
     * Display the crypto difference history to buy page.
     *
    ...
    public function index(Request $request)
    {
        ...
        // Query difference history for current user and to buy tasks with pagination
        $perPage = 10; // Number of records per page
        $query = DifferenceHistory::where('usersId', $user->id)
            ->where('taskType', 'to buy')
        ...
        foreach ($differenceHistory as $index => $record) {
            ...
            $differenceHistoryWithMovement[] = [
                ...
                'current_cash_value' => $record->toBuyCurrentCashValue,
                'starting_coin_value' => $record->toBuyStartingCoinValue,
        ...
        return view('crypto-difference-history-to-buy', compact(
            'differenceHistoryWithMovement',
            'differenceHistory',
            'chartData',
            'startDate',
