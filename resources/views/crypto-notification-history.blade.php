...
@section('title') Crypto Notification History @endsection
            ...
            <div class="card-body">
                ...
                @if($notificationHistory->count() > 0)
                                ...
                                @foreach($notificationHistory as $notification)
                                    ...
                                    @if($task->taskType === 'to sell')
                                        <tr style="background: linear-gradient(135deg, #d4edda 0%, #c3e6cb 100%); border-left: 4px solid #28a745;">
                                            <td><strong>{{ strtoupper($task->taskCoin) }}</strong></td>
                                            <td><span class="badge bg-success" style="border-radius: 20px; padding: 8px 16px;">To Sell</span></td>
                                            <td>{{ $date->format('F j, Y') }}</td>
                                            <td>{{ $date->format('g:iA') }}</td>
                                            <td>{{ number_format($task->currentCoinValue, 8) }} {{ strtoupper($task->taskCoin) }}</td>
                                            <td>₱{{ number_format($task->startingPhpValue, 2) }}</td>
                                            <td>₱{{ number_format($task->minThreshold, 2) }}</td>
                                            <td>₱{{ number_format($task->intervalThreshold, 2) }}</td>
                                            <td><span class="badge bg-info" style="border-radius: 15px;">{{ $notification->threshold_quotient ?? 'N/A' }}</span></td>
                                            <td><strong>₱{{ number_format($notification->finalAmount, 2) }}</strong></td>
                                            <td><strong class="text-success">₱{{ number_format($notification->difference, 2) }}</strong></td>
                                        </tr>
                                    @elseif($task->taskType === 'to buy')
                                        <tr style="background: linear-gradient(135deg, #f8d7da 0%, #f5c6cb 100%); border-left: 4px solid #dc3545;">
                                            ...
                                            <td>{{ $date->format('F j, Y') }}</td>
                                            <td>{{ $date->format('g:iA') }}</td>
                                            <td>₱{{ number_format($task->toBuyCurrentCashValue, 2) }}</td>
                                            <td>{{ number_format($task->toBuyStartingCoinValue, 8) }} {{ strtoupper($task->taskCoin) }}</td>
                                            <td>₱{{ number_format($task->toBuyMinThreshold, 2) }}</td>
                                            <td>₱{{ number_format($task->toBuyIntervalThreshold, 2) }}</td>
                                            <td><span class="badge bg-info" style="border-radius: 15px;">{{ $notification->threshold_quotient ?? 'N/A' }}</span></td>
                                            <td><strong>{{ number_format($notification->finalAmount, 8) }} {{ strtoupper($task->taskCoin) }}</strong></td>
                                            <td><strong class="text-danger">₱{{ number_format($notification->difference, 2) }}</strong></td>
                                        </tr>
                                    @endif
                                @endforeach
                            </tbody>
                    ...
                    <div class="d-flex justify-content-center mt-4">
                        {{ $notificationHistory->appends(request()->query())->links() }}
                    </div>
                @else
