...
@section('title') Product Triggers @endsection
...
@slot('li_3') Triggers @endslot
@slot('title') Product Triggers @endslot
@endcomponent
            ...
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <div>
                        ...
                        @if($product)
                            <p class="card-title-desc">Manage triggers for: <strong>{{ $product->productName }}</strong></p>
                        @else
...
$(document).ready(function() {
    ...
    function loadAvailableTags() {
        ...
        $.ajax({
            url: '{{ route("ecom-products.triggers.available-tags") }}',
            method: 'GET',
            data: {
                id: productId
            },
    ...
    function refreshProductTags() {
        ...
        $.ajax({
            url: '{{ route("ecom-products.triggers") }}',
            method: 'GET',
            data: {
                id: productId,
                refresh: true
    ...
    $('#confirmDeleteTag').click(function() {
        ...
        $.ajax({
            url: '{{ route("ecom-products.triggers.delete-tag", ["id" => ":id"]) }}'.replace(':id', tagId),
            method: 'DELETE',
            data: {
                _token: '{{ csrf_token() }}'
            },
    ...
    $('#saveTriggerTags').click(function() {
        ...
        $.ajax({
            url: '{{ route("ecom-products.triggers.save-tags") }}',
            method: 'POST',
            data: {
