...
@slot('title') Edit Package @endslot
@endcomponent
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col-md-8 mb-3">
                            ...
                            <input type="text" class="form-control" id="packageName" name="packageName"
                                   value="{{ $package->packageName }}" placeholder="Enter package name" required>
                        </div>
                        <div class="col-md-4 mb-3">
                            ...
                            <select class="form-select" id="packageStatus" name="packageStatus">
                                <option value="active" {{ $package->packageStatus === 'active' ? 'selected' : '' }}>Active</option>
                                <option value="inactive" {{ $package->packageStatus === 'inactive' ? 'selected' : '' }}>Inactive</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="packageDescription" name="packageDescription" rows="2" placeholder="Optional description">{{ $package->packageDescription }}</textarea>
                    </div>
                </div>
            </div>
                ...
                <div class="card-body price-summary">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" class="form-control" id="calculatedPrice" value="{{ number_format($package->calculatedPrice, 2, '.', '') }}" readonly>
                        </div>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="input-group">
                            ...
                            <input type="number" class="form-control" id="packagePrice" name="packagePrice"
                                   step="0.01" min="0" value="{{ number_format($package->packagePrice, 2, '.', '') }}" required>
                        </div>
...
$(document).ready(function() {
    ...
    selectedItems[{{ $item->variantId }}] = {
        ...
        productType: null,
        variantName: @json($item->variantName),
        storeName: @json($item->storeName),
        unitPrice: {{ floatval($item->unitPrice) }},
        quantity: {{ $item->quantity }},
        imageUrl: null,
        storeColor: getStoreColor(@json($item->storeName))
    };
    @endforeach
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    // Update selected items with product type and images from available products
                    Object.keys(selectedItems).forEach(function(variantId) {
                        response.products.forEach(function(product) {
                            product.variants.forEach(function(variant) {
                                if (variant.variantId == variantId) {
                                    selectedItems[variantId].productType = variant.productType;
                                    if (variant.imageUrl) {
                                        selectedItems[variantId].imageUrl = variant.imageUrl;
                                    }
                                }
                            });
                        });
    ...
    function renderSelectedItems() {
        ...
        itemsArray.forEach(function(item) {
            ...
            let typeBadge = '';
            if (item.productType === 'access') {
                typeBadge = '<span class="badge bg-info text-white type-badge">Access</span>';
            } else if (item.productType === 'ship') {
                typeBadge = '<span class="badge bg-warning text-dark type-badge">Ship</span>';
            }
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    toastr.error(response.message || 'Failed to update package', 'Error');
                }
            },
            error: function(xhr) {
    ...
    // Initial render of selected items and search products
    renderSelectedItems();
    updateDiscountInfo();
    searchProducts();
});
