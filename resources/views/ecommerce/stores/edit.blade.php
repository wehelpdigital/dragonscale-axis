...
<div class="row">
    <div class="col-lg-8">
        <div class="card">
            <div class="card-body">
                ...
                <form action="{{ route('ecom-stores.update', $store->id) }}" method="POST" enctype="multipart/form-data" id="storeForm">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="col-md-8">
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control @error('storeDescription') is-invalid @enderror"
                                          ...
                                          rows="4"
                                          placeholder="Enter store description (optional)">{{ old('storeDescription', $store->storeDescription) }}</textarea>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('isActive') is-invalid @enderror"
                                        ...
                                        name="isActive"
                                        required>
                                    <option value="1" {{ old('isActive', $store->isActive) == '1' ? 'selected' : '' }}>Active</option>
                                    <option value="0" {{ old('isActive', $store->isActive) == '0' ? 'selected' : '' }}>Inactive</option>
                                </select>
                                @error('isActive')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="col-md-4">
                            <div class="mb-3">
                                ...
                                <div class="logo-preview-container" id="logoPreviewContainer" onclick="document.getElementById('storeLogo').click()">
                                    @if($store->storeLogo)
                                        <div class="placeholder-content d-none" id="logoPlaceholder">
                                        ...
                                        <img src="{{ asset($store->storeLogo) }}" alt="Logo Preview" id="logoPreview">
                                    @else
                                ...
                                <button type="button" class="btn btn-sm btn-outline-danger mt-2 {{ $store->storeLogo ? '' : 'd-none' }}" id="removeLogo">
    ...
    <div class="col-lg-4">
        <div class="card">
            <div class="card-body">
                <h5 class="card-title mb-3"><i class="bx bx-info-circle me-2 text-info"></i>Store Info</h5>
                <ul class="list-unstyled mb-0">
                    <li class="mb-2">
                        <span class="text-muted">Created:</span>
                        <span class="ms-2">{{ $store->created_at->format('M d, Y h:i A') }}</span>
                    </li>
                    <li class="mb-2">
                        <span class="text-muted">Last Updated:</span>
                        <span class="ms-2">{{ $store->updated_at->format('M d, Y h:i A') }}</span>
                    </li>
                    <li>
                        <span class="text-muted">Products:</span>
                        <span class="badge bg-info ms-2">{{ $store->active_products_count }} active</span>
                    </li>
                </ul>
            </div>
        </div>
        ...
        <div class="card">
            <div class="card-body">
                <h5 class="card-title mb-3"><i class="bx bx-bulb me-2 text-warning"></i>Tips</h5>
                <ul class="text-muted mb-0" style="padding-left: 1.2rem;">
                    <li class="mb-2">Changing the store name will update all associated products</li>
                    <li class="mb-2">Inactive stores won't appear in product creation forms</li>
                    <li>Removing the logo will use a default placeholder icon</li>
                </ul>
            </div>
        </div>
    </div>
...
$(document).ready(function() {
    ...
    var hasExistingLogo = {{ $store->storeLogo ? 'true' : 'false' }};
    ...
    $('#removeLogo').on('click', function() {
        if (hasExistingLogo && !$('#storeLogo').val()) {
            // Remove existing logo via AJAX
            $.ajax({
                url: '/ecom-stores/{{ $store->id }}/remove-logo',
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}'
                },
                ...
                error: function(xhr) {
                    toastr.error('An error occurred while removing the logo.', 'Error!');
                }
            });
        } else {
            // Just clear the file input
            $('#storeLogo').val('');
            if (hasExistingLogo && !logoRemoved) {
                $('#logoPreview').attr('src', '{{ $store->storeLogo ? asset($store->storeLogo) : "" }}');
            } else {
                $('#logoPreview').addClass('d-none').attr('src', '');
                $('#logoPlaceholder').removeClass('d-none');
                $('#removeLogo').addClass('d-none');
